...
- **2026-08-21 — Remote cloud sessions cannot reach home-lab infrastructure (INFRA-10)**: PR #18 landed every piece of the NAS migration runnable from a sandbox (`Dockerfile`, `docker-compose.yml`, `docs/DEPLOY-NAS.md`), but the deploy itself — the actual point of the work — could not be executed here. Checked directly rather than assumed: no `ssh`/`scp`/`sftp` binary, no `~/.ssh` keys or config, no `tailscale` CLI, and `ip addr`/`ip route` return nothing, so there is no network path from a remote session to the tailnet or the NAS at all. The `synology-mcp` plugin that would bridge this exists in the `claude-code-studio` marketplace but is not enabled in either `.claude/settings.json` (this repo) or `/root/.claude/settings.json` (only `studio-core` is listed in both) — and its endpoint is a `sensitive` `userConfig` (`synology_mcp_url`) held in the OS keychain by design, not the repo, so enabling the plugin alone would not supply a reachable value; the endpoint would still have to clear this environment's egress policy. General rule: before scoping work that ends in "now execute this against home-lab/on-prem/tailnet infrastructure," check whether the current session actually has a network path to it (an enabled MCP server wired to it, SSH access, tailnet membership) *before* doing the surrounding work — don't discover the dead end only after everything leading up to it is done. When the gap exists, say so plainly and hand off to a local session rather than let the task look further along than it is.
